...
marketing1 <- read.table("C:/ITWILL/2_Rwork/Part-II/marketing.txt", sep = "\n", encoding = "UTF-8")
str(marketing1)
...
str(marketing2) # 472개의 문장 / 1문장이 벡터

# 2. 줄 단위 단어 추출
#----------------------------------------------------
# Map()함수 이용 줄 단위 단어 추출 
lword <- Map(extractNoun, marketing2)  # Map(func, document)
...
lword <- unique(lword) # 중복제거 방법 1(전체 대상)
length(lword) # [1] 353(119개 제거)

# lword <- sapply(lword, unique) # 중복제거 방법 2(줄 단위 대상) 
...
} ## &&와 &의 차이점 객체 전체에 적용하는지 첫번째 객체에만 적용하는지 
...
# head(lword,3) # list에 head를 적용하면 list[[1:6]][1:length(list[[1:6]])]
# head(lword_final)
...
tranrules <- apriori(wordtran, parameter=list(supp=0.25, conf=0.05)) # 지지도:0.25 신뢰도:0.05
