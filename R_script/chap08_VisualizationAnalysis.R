...
# lattice 패키지의 주요 함수
# xyplot(): 산점도, barchart(): 막대차트(이산변수), dotplot(): 이산변수시각화,  cloud(): 4차원의 형태로 시각화, 
# histogram(): 도수분포도(각도수별 출현비율을 밀도로 표현), densityplot(), coplot()
###########################################
available.packages()
...
# | score의 도메인 수 만큼 격자가 생겨남 (group 효과)
summary(Chem97$gcsescore)
table(Chem97$score)

# 2.densityplot(~x축 | 집단변수(조건), dataframe, groups=변수)
...
# layout = c(,) 출력될 차트 개수에 맞게 행렬을 지정해야함
...
# 지진발생 위치(위도와 경로) 
xyplot(lat~long, data=quakes, pch=".")  #pch 포인터를 아무거나 선택가능
# 그래프를 변수에 저장
...
convert <- transform(quakes, depth2=factor(depth2)) # subset같은 기능
head(convert)
xyplot(lat~long | depth2, data=convert)
# : 깊이 별 어느 위경도에서 지진이 발생했는지 "위치"를 찾는 데이터
...
coplot(lat~long | depth, data=quakes, overlap=0.5) # 겹치는 구간 : 0.5
...
qplot(hwy, data=mpg, fill=drv, binwidth=5) # binwidth 옵션 적용 // binwidth기본값 = 1
...
table(mtcars$carb)
# num(동일색에 농도의 변화) vs factor( 집단으로 구분 숫자별 다른 색)
...
p <- ggplot(data = galton, aes(x = child, y = parent))
p + geom_point()

p + geom_count() 
p + geom_count() + geom_smooth()
...
# qplot(wt, mpg, data=mtcars, size=qsec) # geom="point" : 산점도 (이산형)
...
# "l" : 밀도 1을 기준으로 꽉찬 막대차트 ( ideal 과 Fair을 비교 )
...
library(ggmap) # get_stamenmap() : 다른 서버에서 지도를 가져오는
...
map <- get_stamenmap(seoul, zoom=10,  maptype='terrain')#'toner-2011')
...
map <- get_stamenmap(daegu, zoom=12,  maptype='watercolor')

# [ 단계 1 ] dataset 가져오기
setwd("C:/ITWILL/2_Rwork/Part-II")
pop <- read.csv("population201901.csv")
str(pop) # LAT 위도 LON 경도 
...
# [ 단계 4 ] 레이어2 : 각 지역별 포인트 추가
layer2 <- layer1 + geom_point(data = df, aes(x = lon, y = lat,
                          color = factor(tot_pop),
                          size = factor(tot_pop)))


# [ 단계 5 ] 레이어3 : 각 지역별 포인트옆 지명 표현
layer3 <- layer2 + geom_text(data = df, aes(x = lon+0.08,
                                            y = lat+0.08,
                                            label = region),
                             size = 3 ) # 표현할 지역명 글씨 사이즈 

# [ 단계 6 ] 지도 이미지 file save
